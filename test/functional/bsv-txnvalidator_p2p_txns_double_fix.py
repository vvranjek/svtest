...
class TxnValidatorP2PTxnsTest(BitcoinTestFramework):
    ...
    def make_transactions(self, txtype, num_txns, outs_per_tx, create_double_spends=False):
        ...
        fund_txn_num_vouts = outs_per_tx
        ...
        for i in range(len(fund_txns)):
            for fund_txn_vout_idx in range(fund_txn_num_vouts):
                ...
                if create_double_spends and len(spend_txs) < num_txns // 2:
                    # The first double spend (n=0, n=0) should be ignored because it creates an invalid transaction instead of a doublespend
                    if (fund_txn_vout_idx == 0):
                        bad_txs += 1
                        continue
                    ...
                    spend_tx.rehash()
                    print("Double: %s" % spend_tx.hash)
                    #print("Double: %s" % spend_tx)
                else:
                    print("Single: %s" % spend_tx.hash)

                # The first double spend should be ignored because it creates an invalid transaction instead of a doublespend

                #if (create_double_spends and len(spend_txs) < num_txns // 2 and fund_txn_vout_idx == 0):
                #    print("Ignoring bad tx")
                 #   bad_txs += 1
                  #  continue

                #print("loop: %d/%d" % (i, fund_txn_vout_idx))

                spend_txs.append(spend_tx)

        return spend_txs, bad_txs
    ...
    def run_test(self):
        ...
        def get_mempool():
            ...
            print("Mempool len: %d" % len(mempool))
            return mempool
        ...
        def runTestWithParams(sync_node_type, description, args, txtype, number_of_txns, number_of_peers, timeout, shuffle_txns=False):
            ...
            result_txns, bad_txs = self.make_transactions(txtype, number_of_txns, outs_per_tx, create_double_spends)
            ...
            if create_double_spends:
                ...
                # This calculation may be hard to understand. When doing double spend transactions, we need to consider that
                # the first processed tx among fund_txn_num_vouts will be accepted and others will be treated as double spends

                # bad_txs can also be calculated as show below, but we use the counted version from make_transactions()
                #bad_txs = int(number_of_txns/2/outs_per_tx)

                expected_txns_num = number_of_txns/2 - number_of_txns/outs_per_tx + bad_txs
            else:
            ...
            #if (create_double_spends):
            #    number_of_txns = number_of_txns - 1
            assert_equal(len(spend_txns), number_of_txns - bad_txs)
            ...
            with run_connection(description, callback_connections):
                ...
                #wait_until(lambda: len(self.nodes[0].getrawmempool()) >= expected_txns_num-bad_txs, timeout=timeout)
                wait_until(lambda: len(get_mempool()) >= expected_txns_num, timeout=timeout)
                # Wait a while longor in case if any more unwanted txs arrive
                time.sleep(1.0)
        ...
        for test_case in test_cases:
           ...
           # while (True):
            ...
            #runTestWithParams(test_case[0], test_case[1], test_case[2], test_case[3], 10, test_case[5], test_case[6], test_case[7])
