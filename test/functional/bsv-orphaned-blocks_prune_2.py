...
class OrphanedBlocks(BitcoinTestFramework):
    ...
    def reconsider_block(self):
        ...
        def run_connection(test_node, title):
            ...
            #connections.append(
            #    mininode.NodeConn('127.0.0.1', p2p_port(0), self.nodes[0], mininode2))
            #mininode1.add_connection(connections[1])
        ...
        print("Connecting mininode")
        mininode1 = mininode.NodeConnCB()
        print("Done")
        def on_getdata(conn, message):
            print("GEEEEEEEEET DATAAAAAAAA")

        node1.on_getdata = on_getdata


        with run_connection(mininode1, "0 fields"):
            print("Run connection")
            #mininode1.wait_for_verack()
            #mininode1.wait_for_disconnect()
